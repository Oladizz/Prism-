...
const Portfolio: React.FC = () => {
    ...
    const portfolioStats = useMemo(() => {
        ...
        // Mock change data
        const portfolioChange = totalBalance * 0.052; 
        const portfolioChangePercent = 5.2;
            ...
            <div className="space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    {isLoading ? Array.from({length: 4}).map((_, i) => <Skeleton key={i} className="h-[105px] rounded-3xl" />) : <>
                        <SummaryCard title="Total Value" value={`$${portfolioStats.totalBalance.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`} icon={<WalletIcon />} change={`+$${portfolioStats.portfolioChange.toFixed(2)} (+${portfolioStats.portfolioChangePercent}%)`} changeColor="text-green-400" />
