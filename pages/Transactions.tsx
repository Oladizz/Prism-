...
const TransactionRow: React.FC<{ transaction: Transaction }> = ({ transaction }) => {
  ...
  return (
    <tr className="border-b border-border hover:bg-black/20"><td className="p-4"><p className="font-semibold text-white text-sm">{date}</p><p className="text-xs text-gray-400">{time}</p></td><td className="p-4"><div className="flex items-center gap-3"><Icon className="w-8 h-8" /><div><p className="font-semibold text-white">{transaction.asset.name}</p><p className="text-sm text-gray-400">{transaction.asset.ticker}</p></div></div></td><td className="p-4"><p className="font-semibold text-white font-mono text-sm">{transaction.amountCrypto.toFixed(4)} {transaction.asset.ticker}</p><p className="text-xs text-gray-400 font-mono">${transaction.amountUSD.toLocaleString()}</p></td><td className="p-4"><TypeBadge type={transaction.type} /></td><td className="p-4">{renderFromTo()}</td><td className="p-4"><span className={`px-2 py-1 text-xs font-semibold rounded-full ${getStatusClass(transaction.status)}`}>{transaction.status}</span></td></tr>
  );
};
